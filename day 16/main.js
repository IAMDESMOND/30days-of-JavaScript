...
let studentsNames = JSON.stringify(student, ['firstName', 'lastName', 'skills'], 4)
...
for (const user in users
    ...
    if (Object.hasOwnProperty.call(users
    , user)) {
        const obj = users
    [user];
        let skillsLength = obj.skills.length
        if (count < skillsLength) {
            count = skillsLength
            maxSkills = user
        }
    }  
